...
class ChatHubService {
  ...
  private async invokeWhenConnected<T = unknown>(
    method: string,
    ...args: unknown[]
  ): Promise<T> {
    console.log(
      '[DEBUG] invokeWhenConnected called with method:',
      method,
      'args:',
      args
    )
    ...
    const result = conn.invoke(method, ...args)
  ...
  async joinLivestream(livestreamId: string) {
    ...
    const result = await this.invokeWhenConnected(
      'JoinLivestreamChatRoom',
      livestreamId
    )
  ...
  async startViewingLivestream(livestreamId: string) {
    console.log(
      '[DEBUG] chatHub.startViewingLivestream called with:',
      livestreamId
    )
    const result = await this.invokeWhenConnected(
      'StartViewingLivestream',
      livestreamId
    )
    console.log('[DEBUG] startViewingLivestream result:', result)
    console.log(
      `[DEBUG]  Viewer should now be in group: livestream_viewers_${livestreamId}`
    )

    // Add debugging for group membership verification
    setTimeout(async () => {
      console.log(
        '[DEBUG]  Checking if viewer is properly in group after 2 seconds...'
      )
      try {
  ...
  async stopViewingLivestream(livestreamId: string) {
    try {
      await this.invokeWhenConnected('StopViewingLivestream', livestreamId)
    } catch {
      /* ignore */
    }
  }
  ...
  async leaveLivestream(livestreamId: string) {
    ...
    if (this.connection.state === HubConnectionState.Connected) {
      try {
        await this.connection.invoke('LeaveLivestreamChatRoom', livestreamId)
      } catch {}
    }
  }
  ...
  onReceiveLivestreamMessage(cb: (payload: LivestreamMessagePayload) => void) {
    ...
    const handler = (raw: RawMsg) => {
      const payload: LivestreamMessagePayload = {
        ...
        senderType:
          raw.senderType ?? raw.SenderType ?? raw.senderRole ?? raw.SenderRole,
        senderAvatarUrl:
          raw.senderAvatarUrl ??
          raw.SenderAvatarUrl ??
          raw.avatarUrl ??
    ...
    this.connection?.on(
      ...
      handler as unknown as (...args: never[]) => void
    )
    this.connection?.on(
      'receivelivestreammessage' as unknown as string,
      handler as unknown as (...args: never[]) => void
    )
  }
  ...
  onUserJoined(cb: (payload: UserPresencePayload) => void) {
    ...
    const handler = (raw: RawPresence) => {
      const payload: UserPresencePayload = {
        ...
        timestamp:
          this.toStr(raw?.timestamp ?? raw?.Timestamp) ??
    ...
    this.connection?.on(
      'userjoined' as unknown as string,
      handler as unknown as (...args: never[]) => void
    )
  }
  ...
  onUserLeft(cb: (payload: UserPresencePayload) => void) {
    ...
    this.connection?.on(
      'userleft' as unknown as string,
      handler as unknown as (...args: never[]) => void
    )
  }

  onViewerStats(
    cb: (payload: ViewerStatsPayload) => void
  ): (() => void) | void {
    ...
    const handler = (raw: RawStats) => {
      ...
      const normalized: ViewerStatsPayload = {
        ...
        viewersByRole: (raw?.viewersByRole ??
          raw?.ViewersByRole ??
        ...
        customerViewers: Number(
          raw?.customerViewers ?? raw?.CustomerViewers ?? NaN
        ),
        maxCustomerViewer: Number(
          raw?.maxCustomerViewer ?? raw?.MaxCustomerViewer ?? NaN
        ),
      ...
      if (normalized.livestreamId) {
        ...
        const currPeak =
          this.peakCustomerByLive.get(normalized.livestreamId) ?? 0
    ...
    return () => {
      try {
        conn.off('ReceiveViewerStats', handlerFn)
      } catch {}
      try {
        conn.off('receiveviewerstats' as unknown as string, handlerFn)
      } catch {}
    }
  }
  ...
  async updateProductStock(
    ...
    return this.invokeWhenConnected(
      'UpdateProductStock',
      livestreamId,
      productId,
      variantId,
  ...
  async pinProduct(
    ...
    try {
      return await this.invokeWhenConnected(
        'PinProduct',
        livestreamId,
        productId,
        variantId,
  ...
  async removeProductFromLivestream(
    livestreamId: string,
    productId: string,
    variantId: string | null
  ) {
  ...
  async updateLivestreamProductById(
    id: string,
    price: number,
    stock: number,
    isPin: boolean
  ...
  async updateLivestreamProductStockById(id: string, newStock: number) {
    console.log(
      '[DEBUG] updateLivestreamProductStockById called with id:',
      id,
      'newStock:',
      newStock
    )
    const result = await this.invokeWhenConnected(
      'UpdateLivestreamProductStockById',
      id,
      newStock
    )
  ...
  async testBroadcastToAll(livestreamId: string, message: string) {
    console.log('[DEBUG] 🧪 Testing broadcast to ALL connections:', {
      livestreamId,
      message,
    })
    return await this.invokeWhenConnected(
  ...
  async softDeleteLivestreamProductById(
    id: string,
    reason = 'Removed by seller'
  ) {
    return this.invokeWhenConnected(
      'SoftDeleteLivestreamProductById',
      id,
      reason
    )
  }
    ...
    this.connection?.on(
      ...
      (raw: Record<string, unknown>) => {
        const payload: ProductStockUpdatedEvent = {
          livestreamId:
            this.toStr(raw?.LivestreamId ?? raw?.livestreamId) ?? '',
  ...
  onProductPinStatusChanged(
    cb: (payload: ProductPinStatusChangedEvent) => void
  ) {
    this.connection?.off('ProductPinStatusChanged')
    this.connection?.on(
      'ProductPinStatusChanged',
      (raw: Record<string, unknown>) => {
  ...
  onLivestreamProductPinStatusChanged(
    cb: (payload: ProductPinStatusChangedEvent) => void
  ) {
    this.connection?.off('LivestreamProductPinStatusChanged')
    this.connection?.on(
      'LivestreamProductPinStatusChanged',
      (raw: Record<string, unknown>) => {
        const payload: ProductPinStatusChangedEvent = {
          livestreamId:
            this.toStr(raw?.LivestreamId ?? raw?.livestreamId) ?? '',
  ...
  onLivestreamProductStockUpdated(
    cb: (payload: ProductStockUpdatedEvent) => void
  ) {
    this.connection?.off('LivestreamProductStockUpdated')
    this.connection?.on(
      'LivestreamProductStockUpdated',
      (raw: Record<string, unknown>) => {
        const payload: ProductStockUpdatedEvent = {
          livestreamId:
            this.toStr(raw?.LivestreamId ?? raw?.livestreamId) ?? '',
  ...
  onLivestreamProductSoftDeleted(
    cb: (payload: ProductEventBase & { reason?: string }) => void
  ) {
    this.connection?.off('LivestreamProductSoftDeleted')
    this.connection?.on(
      'LivestreamProductSoftDeleted',
      (raw: Record<string, unknown>) => {
  ...
  onPinnedProductsUpdated(cb: (payload: PinnedProductsUpdatedEvent) => void) {
    ...
    this.connection?.on(
      'PinnedProductsUpdated',
      (raw: Record<string, unknown>) => {
        const pinned = this.getArrayField(
          raw,
          'PinnedProducts',
          'pinnedProducts'
        )
  ...
  onLivestreamProductsLoaded(
    cb: (payload: LivestreamProductsLoadedPayload) => void
  ) {
    this.connection?.off('LivestreamProductsLoaded')
    // also listen to lowercase variant to be safe
    try {
      this.connection?.off('livestreamproductsloaded' as unknown as string)
    } catch {}
    ...
    this.connection?.on(
      'LivestreamProductsLoaded',
      handler as unknown as (...args: never[]) => void
    )
    this.connection?.on(
      'livestreamproductsloaded' as unknown as string,
      handler as unknown as (...args: never[]) => void
    )
  }

  onMaxCustomerViewerUpdated(
    cb: (payload: {
      livestreamId: string
      newMaxCustomerViewer: number
      previousMax: number
      viewerType?: string
      timestamp?: string
      message?: string
    ...
    this.connection?.on(
      'MaxCustomerViewerUpdated',
      (raw: Record<string, unknown>) => {
        const payload = {
          livestreamId:
            this.toStr(raw?.LivestreamId ?? raw?.livestreamId) ?? '',
          newMaxCustomerViewer:
            this.toNum(
              raw?.NewMaxCustomerViewer ?? raw?.newMaxCustomerViewer
            ) ?? 0,
        ...
        if (payload.livestreamId) {
          const currPeak =
            this.peakCustomerByLive.get(payload.livestreamId) ?? 0
          this.peakCustomerByLive.set(
            payload.livestreamId,
            Math.max(currPeak, payload.newMaxCustomerViewer)
          )
        }
        cb(payload)
      }
  ...
  getEndOfLiveSummary(livestreamId: string) {
    ...
    const peakCustomerViewers =
      this.peakCustomerByLive.get(livestreamId) ??
      lastStats?.maxCustomerViewer ??
      0
    return {
      livestreamId,
      uniqueViewers,
