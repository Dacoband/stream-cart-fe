            ...
            <div className="flex-1 overflow-y-auto min-h-0">
              {loadingRooms ? (
                ...
                filteredRooms.map((room) => (
                  ...
                  >
                    <div className="font-medium truncate">{room.shopName}</div>
                    <div
...
const ChatWithShop: React.FC<ChatWithShopProps> = (props) => {
  ...
  // Guard when external toggles (e.g., InforShop button) set open=true without login
  useEffect(() => {
    if (props.open && !user) {
      toast.error("Vui lòng đăng nhập.");
      // Close again to avoid flashing UI
      props.setOpen(false);
      router.push(
  ...
  return (
    <>
      {/* Floating toggle button (does not require ChatProvider) */}
      <button
        type="button"
        title="Chat With Shop"
        onClick={handleChatToggle}
        className={`fixed bottom-5 right-5 z-50 w-14 h-14 flex items-center justify-center rounded-full shadow-lg transition-all duration-300 hover:scale-110 hover:rotate-6 ${
          props.open
      ...
      {/* Only mount chat provider and UI when open */}
      {props.open && (
