...
function SearchPageInner() {
  ...
  const [filters, setFilters] =
    React.useState<LocalFilterState>(initialFilters);
  ...
  const buildQuery = React.useCallback(
    (overrides: Partial<{ q: string }> = {}) => {
      ...
      if (filters.inStockOnly !== undefined)
        qp.set("inStockOnly", String(filters.inStockOnly));
      if (filters.onSaleOnly !== undefined)
        qp.set("onSaleOnly", String(filters.onSaleOnly));
  ...
  const fetchPage = React.useCallback(
    async (page: number, replace = false) => {
      ...
      try {
        ...
        setMeta(newMeta);
        setProducts((prev) =>
          replace ? newMeta.items : [...prev, ...newMeta.items]
        );
        ...
        const msg =
          typeof e === "object" && e !== null && "message" in e
            ? (e as ErrLike).message || "Lỗi tìm kiếm"
            : "Lỗi tìm kiếm";
        setError(msg);
      } finally {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const items = ((res || []) as RawShopProduct[])
          .slice(0, 12)
          .map((p: RawShopProduct) => ({
            id: p.id,
            ...
            discountPercentage:
              p.basePrice && p.finalPrice && p.basePrice > p.finalPrice
                ? Math.round((1 - p.finalPrice / p.basePrice) * 100)
                : undefined,
        ...
        if (!cancelled) setLoadingShopProducts(false);
      }
    })();
    return () => {
      cancelled = true;
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (
          ...
          meta &&
          products.length < meta.totalCount &&
          !initialLoading &&
          !loadingMore
        ) {
  ...
  const ProductCard: React.FC<{
    ...
    return (
      <Link href={`/product/${product.id}`} className="group block h-full">
        <div
          className={`flex flex-col h-full bg-white rounded-none border transition-all duration-300 hover:shadow-xl overflow-hidden ${
          ...
          <div className="flex flex-col flex-1 px-3 pt-2 pb-3">
            <h4
              className={`font-medium text-gray-900 line-clamp-2 mb-2 ${
                compact ? "text-[11px] min-h-[36px]" : "text-sm min-h-[48px]"
              }`}
            >
              {product.productName}
            </h4>
            <div className="mt-auto">
              <div className="flex items-end justify-between gap-4 mb-2">
                <span
                  className={`font-semibold text-[#7FB800] ${
      ...
      <div className="grid grid-cols-12 gap-6">
        <aside className={`col-span-12 md:col-span-3 xl:col-span-2 text-xs`}>
          <div className="flex items-center gap-2 mb-5 font-semibold text-slate-800 dark:text-slate-200 text-sm">
            <Filter size={16} className="text-[#B0F847]" />{" "}
            <span className="uppercase tracking-wide">Bộ lọc tìm kiếm</span>
          </div>
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 uppercase">
                Khoảng giá
              </div>
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center gap-2 text-slate-700 dark:text-slate-300 text-[11px] cursor-pointer">
                <input
                  type="checkbox"
                  className="accent-[#B0F847]"
              ...
              <label className="flex items-center gap-2 text-slate-700 dark:text-slate-300 text-[11px] cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFilters((f) => ({
                      ...f,
                      onSaleOnly: e.target.checked || undefined,
                    }))
                  }
                />
                Đang giảm giá
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full text-xs bg-white text-black py-1 px-2 rounded border border-gray-300"
              >
            ...
            <div className="flex gap-2 pt-1">
              <Button
                size="sm"
                variant="secondary"
                className="flex-1 bg-[#9FE040] text-black hover:brightness-95 hover:text-black font-semibold"
              ...
              <Button
                size="sm"
                variant="ghost"
                className="flex-1 text-slate-500 hover:text-black"
        ...
        <section className="col-span-12 md:col-span-9 xl:col-span-10 space-y-4 min-h-[400px]">
              ...
              <div className="flex items-center gap-4 w-full sm:w-auto">
                <div className="relative w-20 h-20 rounded-full overflow-hidden border-2 border-[#2c3035] bg-[#2c3035]">
                  {shops[0].logoURL || shops[0].logoUrl ? (
                    <Image
                      src={(shops[0].logoURL || shops[0].logoUrl)!}
                      alt={shops[0].shopName}
                      fill
                ...
                <div className="space-y-1 min-w-0">
                  ...
                  <div className="text-[11px] text-slate-900 flex flex-wrap gap-x-3 gap-y-1">
                    ...
                    {shops[0].completeRate !== undefined && (
                      <span>{shops[0].completeRate}% phản hồi</span>
                    )}
                  </div>
                  <div className="text-[11px] text-slate-500 line-clamp-2 max-w-md">
                    {shops[0].description || "Không có mô tả"}
                  </div>
                </div>
              </div>
              ...
              {[
                ...
                <button
                  ...
                  onClick={() => {
                    setFilters((f) => ({
                      ...f,
                      sortBy: o.val === "relevance" ? undefined : o.val,
                    }));
                    router.replace(`/search?${buildQuery()}`);
                  }}
                  className={`px-3 py-1 rounded-sm font-medium border ${
                    (filters.sortBy || "relevance") === o.val
                      ? "bg-[#B0F847] text-black border-[#B0F847]"
                      : "border-transparent bg-[#23272b] text-slate-300 hover:text-[#B0F847]"
                  }`}
                >
                  {o.label}
                </button>
          ...
          {!initialLoading &&
            ...
            q &&
            meta &&
            (meta.totalCount > 0 || shopProducts.length === 0) && (
              <div className="text-xs md:text-sm text-slate-400">
                Tìm thấy{" "}
                <span className="text-[#B0F847] font-medium">
                  {meta.totalCount}
                </span>{" "}
                sản phẩm cho &quot;{q}&quot;
              </div>
          ...
          {!initialLoading &&
            ...
            shopProducts.length === 0 && (
              <div className="p-8 text-center border rounded bg-[#1f2226] text-slate-300">
                Không tìm thấy kết quả cho “{q}”.
              </div>
            )}
          <div className="grid gap-6 grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
            {products
              .filter((p) => !shopProducts.some((sp) => sp.id === p.id))
              .map((p) => (
            ...
            <div className="space-y-2 mt-8">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <h3 className="text-sm font-semibold text-shadow-slate-700">
                  ...
                  shopProducts.length > 0 ? (
                    <>
                      Không tìm thấy sản phẩm khớp “{q}”. Gợi ý sản phẩm từ cửa
                      hàng liên quan
                    </>
                  ) : (
              ...
              {!loadingShopProducts && shopProducts.length === 0 && (
