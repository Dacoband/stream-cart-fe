  ...
  React.useEffect(() => {
    const fetchUserRoleData = async () => {
      ...
      try {
        ...
        let customerCount = 0, sellerCount = 0, moderatorCount = 0;
        ...
        const fallbackData: UserRoleData[] = [
          { role: "Customer", count: 850, color: "#3B82F6", icon: <Users className="w-4 h-4" />, roleId: 1 },
          { role: "Seller", count: 320, color: "#10B981", icon: <Store className="w-4 h-4" />, roleId: 2 },
          { role: "Moderator", count: 25, color: "#F59E0B", icon: <Shield className="w-4 h-4" />, roleId: 3 }
        ];
    ...
    <Card className="p-6 h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Tổng quan về các loại tài khoản</p>
        </div>
      </div>
