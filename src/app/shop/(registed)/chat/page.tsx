...
import { ArrowLeft, Image, Info, MoreVertical, Paperclip, Phone, Search, Send, Smile, Video } from 'lucide-react';
...
export default function ShopChatPage() {
  ...
  const filteredRooms = searchText
    ? rooms.filter(room =>
      (room.userName || room.shopName || '').toLowerCase().includes(searchText.toLowerCase())
    )
    : rooms;
      ...
      <aside className="w-80 bg-white border-r h-full border-gray-200 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {filteredRooms.map(r => {
            ...
            const time = formatLastSeen((r.lastMessage?.sentAt || r.lastMessageAt));
            return (
              <div
                key={r.id}
                onClick={() => openRoom(r)}
                className={`p-4 hover:bg-gray-50 cursor-pointer transition-colors border-l-4 ${selectedRoom?.id === r.id
      ...
      <main className="flex-1 flex flex-col bg-white h-full overflow-hidden">
        {selectedRoom ? (
          <>
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                      {customerAvatarUrl || selectedRoom.userAvatarUrl ? (
                        <img
                          ...
                          alt={selectedRoom.userName || selectedRoom.shopName || 'User'}
                          className="w-full h-full object-cover"
                        />
                      ) : (
                        <span className="text-sm font-semibold">
                          {(selectedRoom.userName || selectedRoom.shopName || 'U').charAt(0).toUpperCase()}
                        </span>
                      )}
                    </div>
                  </div>
                  <div>
                    <h2 className="font-semibold text-gray-900">
                      {selectedRoom.userName || selectedRoom.shopName || 'Không tên'}
                    </h2>
                    <p className="text-sm text-gray-500">ID phòng: {selectedRoom.id}</p>
                  </div>
                </div>

                <div className="flex cursor-pointer items-center space-x-2">
                  <button type='button' title='Phone' className="p-2 hover:bg-gray-100 rounded-full transition-colors">
                    <Phone className="w-5 h-5 text-gray-600" />
                  </button>
                  <button type='button' title='Video' className="p-2 hover:bg-gray-100 rounded-full transition-colors">
                    <Video className="w-5 h-5 text-gray-600" />
                  </button>
                  <button type='button' title='Info' className="p-2 hover:bg-gray-100 rounded-full transition-colors">
                    <Info className="w-5 h-5 text-gray-600" />
                  </button>
                  <button type='button' title='MoreVertical' className="p-2 hover:bg-gray-100 rounded-full transition-colors">
            ...
            {/* Messages Area */}
            <div className="flex-1 p-3 pb-40 overflow-y-auto min-h-0" style={{backgroundColor: '#f8fafc'}}>
              {messages.length === 0 && (
                ...
                return (
                  <React.Fragment key={m.id}>
                    ...
                    <div className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                      <div className={`flex items-end mb-5 space-x-2 max-w-[70%] ${mine ? 'flex-row-reverse space-x-reverse' : ''}`}>
                        {!mine && (
                        ...
                        <div className={`relative px-4 py-2 rounded-2xl ${mine
            ...
            <div className="fixed bottom-0 w-[68%] p-4 border-t border-gray-200 bg-white">
              <div className="flex items-center space-x-2">
                <button type='button' title='Paperclip' className="p-2 hover:bg-gray-100 rounded-full transition-colors">
                  <Paperclip className="w-5 h-5 text-gray-500" />
                </button>
                <button type='button' title='Image' className="p-2 hover:bg-gray-100 rounded-full transition-colors">
                ...
                <div className="flex-1 relative">
                  ...
                  />
                  <button type='button' title='Smile'
