...
const ChatUI: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const avatar = (parsed['avatarUrl'] as string) || (parsed['avatarURL'] as string) || (parsed['imageUrl'] as string) || (parsed['profileImageUrl'] as string) || (parsed['avatar'] as string) || '';
  ...
  useEffect(() => {
    const qsShopId = typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('shopId') : null;
    if (!qsShopId) return;
    let mounted = true;
    (async () => {
      try {
        ...
        if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_DEBUG_API === 'true') console.debug('[CustomerChat] getShopDetail failed', e);
      }
    })();
    ...
    <div className="flex h-[calc(100vh-4rem)]">
      ...
      <main className="flex-1 flex flex-col max-w-3xl mx-auto">
        <div className="p-4 border-b">
          ...
          <div className={`text-xs ${connected ? 'text-green-600' : 'text-red-600'}`}>{connected ? 'Đã kết nối' : 'Mất kết nối'}</div>
        </div>
                ...
                <React.Fragment key={m.id}>
                  ...
                  <div className={`flex items-end mb-2 ${mine ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[70%] px-3 py-2 rounded-2xl ${mine ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-900'}`}>
                      ...
                      <div className={`text-[10px] mt-1 ${mine ? 'text-blue-100' : 'text-gray-400'}`}>{new Date(m.sentAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                    </div>
                    {mine && (
        ...
        <div className="p-3 border-t flex gap-2">
          ...
          />
          <button className="px-4 py-2 bg-blue-600 text-white rounded-full" onClick={onSend}>Gửi</button>
        </div>
      </main>
    </div>
  );
