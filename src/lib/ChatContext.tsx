...
export const ChatProvider: React.FC<{ shopId?: string } & React.PropsWithChildren> = ({ shopId, children }) => {
  ...
  const makeSignature = (m: Partial<ChatMessage>) => {
    ...
    const sentAt = String(m.sentAt ?? '').slice(0, 19); // second precision
  ...
  useEffect(() => {
    const onReceive = (data: Record<string, unknown>) => {
      ...
      if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_DEBUG_API === 'true') {
        console.debug('[ChatProvider] onReceive raw', data, 'mapped', mm, 'currentRoom', currentRoomRef.current);
      }
      // mark isMine if senderUserId equals stored user id
      const uid = currentUserIdRef.current;
      mm.isMine = !!(uid && mm.senderUserId && String(mm.senderUserId) === String(uid));
      ...
      setMessages(prev => {
        ...
        const approxIdx = prev.findIndex(p => (
          ...
          String(p.content ?? '').trim() === String(mm.content ?? '').trim() &&
          within(toTime(p.sentAt), mmTime)
        ));
        if (approxIdx >= 0) {
          // prefer the one with a non-fallback id
          const existing = prev[approxIdx];
          const incomingBetter = mm.id && (!isMillisId(mm.id) || isMillisId(existing.id));
          if (incomingBetter) {
            const copy = [...prev];
            copy[approxIdx] = mm;
            return copy;
    ...
    signalRChatClient.onUserTyping = (d) => console.log('UserTyping', d as { userId?: string; isTyping?: boolean });
  ...
  let currentUserId: string | null = null;
        try {
          ...
          if (raw) currentUserId = (JSON.parse(raw) as { id?: string }).id ?? null;
        } catch {
          currentUserId = null;
        }
  currentUserIdRef.current = currentUserId;
        ...
        try {
          ...
          if (room && currentUserId && String(room.userId) !== String(currentUserId)) {
            // room exists but not for this user; treat as not found so we may create one for current user
            console.debug('[ChatProvider] found room but belongs to different user, will create new for current user', { foundUserId: room.userId, currentUserId });
            room = null;
          }
        } catch (e) {
        ...
        try {
          ...
          if (raw) {
            ...
            if (ud.shopId || (ud.ShopId as unknown)) {
              ...
              const role = (ud.role ?? ud.Role ?? ud.roleName ?? ud.Type) as string | undefined;
              if (role && String(role).toLowerCase().includes('shop')) isShopUser = true;
            }
            // debug: show parsed userData to help troubleshoot role/shopId
            console.debug('[ChatProvider] userData parsed for role/shopId detection', { role: ud.role ?? ud.Role, shopId: ud.shopId ?? ud.ShopId });
          }
        } catch {
          isShopUser = false;
        }
        ...
        if (!room && !isShopUser) {
          ...
          if (!room) {
            ...
            console.debug('[ChatProvider] creating chat room', { shopId, currentUserId });
        ...
        if (room) {
          setCurrentRoomId(room.id);
            ...
            const normalized = (msgs.items ?? []).map((m: unknown) => {
              ...
              if (!mapped.chatRoomId) mapped.chatRoomId = (rec['chatRoomId'] as string) ?? (rec['roomId'] as string) ?? room.id;
              // ensure id fallback
            ...
            if (ok) {
              try {
                ...
                if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_DEBUG_API === 'true') console.debug('[ChatProvider] signalR joinChatRoom', { roomId: room.id, joined });
              } catch (e) {
                console.error('[ChatProvider] signalR joinChatRoom failed', e);
              }
              try {
                const restJoin = await chatApi.joinSignalRChatRoom(room.id);
                if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_DEBUG_API === 'true') console.debug('[ChatProvider] REST joinSignalRChatRoom', { roomId: room.id, restJoin });
              } catch (e) {
                console.error('[ChatProvider] REST joinSignalRChatRoom failed', e);
              }
            }
        }
      } catch (err) {
    ...
    const startPolling = () => {
      ...
      pollId = window.setInterval(async () => {
        try {
          ...
          const resp = await chatApi.getChatMessages(currentRoomRef.current, 1, 50);
          const items = (resp.items ?? []).map((m: unknown) => {
            const rec = m as Record<string, unknown>;
            const mapped = mapSignalRToChatMessage(rec);
            if (!mapped.chatRoomId) mapped.chatRoomId = (rec['chatRoomId'] as string) ?? (rec['roomId'] as string) ?? currentRoomRef.current!;
            if (!mapped.id) mapped.id = (rec['id'] as string) ?? (rec['_id'] as string) ?? `${Date.now()}`;
            const uid = currentUserIdRef.current;
            mapped.isMine = !!(uid && mapped.senderUserId && String(mapped.senderUserId) === String(uid));
            return mapped as ChatMessage;
          });
          // sort ascending
          items.sort((a, b) => (Date.parse(a.sentAt ?? '') || 0) - (Date.parse(b.sentAt ?? '') || 0));
          ...
          setMessages(prev => {
            ...
            for (const it of items) {
              ...
              const sigIdx = merged.findIndex(p => makeSignature(p) === itSig);
              if (sigIdx >= 0) {
                // replace if existing has fallback millis id and incoming looks definitive
                if (isMillisId(merged[sigIdx].id) && it.id && !isMillisId(it.id)) {
                  merged[sigIdx] = it;
                }
                continue;
              }

              const approxIdx = merged.findIndex(p => (
                String(p.senderUserId ?? '') === String(it.senderUserId ?? '') &&
                String(p.content ?? '').trim() === String(it.content ?? '').trim() &&
                within(toTime(p.sentAt), toTime(it.sentAt))
              ));
              if (approxIdx >= 0) {
                // prefer the one with non-fallback id
                const existing = merged[approxIdx];
                const incomingBetter = it.id && (!isMillisId(it.id) || isMillisId(existing.id));
            ...
            merged.sort((a, b) => (Date.parse(a.sentAt ?? '') || 0) - (Date.parse(b.sentAt ?? '') || 0));
            return merged;
          });
        } catch {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_DEBUG_API === 'true') console.debug('[ChatProvider] sendMessage request (REST-first)', { roomId: currentRoomId, content, signalRConnected: signalRChatClient.isConnected });
      ...
      if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_DEBUG_API === 'true') console.debug('[ChatProvider] sendMessage via REST result', res);

      // Optionally also notify via SignalR to reduce perceived latency (non-blocking)
      if (signalRChatClient.isConnected) {
        try {
          const ok = await signalRChatClient.sendMessage(currentRoomId, content);
          if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_DEBUG_API === 'true') console.debug('[ChatProvider] notify via SignalR', { roomId: currentRoomId, ok });
        } catch (e) {
          if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_DEBUG_API === 'true') console.debug('[ChatProvider] notify via SignalR failed', e);
        }
      }
    } catch (err) {
  ...
  const setTyping = async (isTyping: boolean) => {
    ...
    try {
      ...
      if (isTyping) {
        ...
        typingTimeout.current = window.setTimeout(() => signalRChatClient.setTypingStatus(currentRoomRef.current ?? '', false), 2000);
      }
    } catch {
      // ignore
    }
  ...
  return (
    <ChatContext.Provider value={{ messages, currentRoomId, sendMessage, setTyping, connected }}>
      {children}
